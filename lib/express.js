...
function getAltNetwork(opts) {
  let network
  let net = coininfo[opts.coin]
  if (opts.testnet) {
    network = net.test
    networkName = 'testnet'
    ...
    throw new Error('CUSTOM option does not supported in altcoins.')
  } else {
    network = net.main
    networkName = 'mainnet'
  }
  return { network.toBitcoinJS(), networkName }
}
...
module.exports = function initialize (adapter, opts) {
  ...
  if (opts.coin) {
    { network, networkName } = getAltNetwork(opts);
  ...
  function addressToScriptId(address) {
    ...
    if (address.startsWith('mona') || address.startsWith('tmona')) {
